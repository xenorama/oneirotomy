...
function assign_mov(m){
  ...
  if (verbose) {
    ...
    outlet(0,"sources","info",thismov.obj.maxclass,"hosting",thismov.movie,"(output_texture "+thismov.texture+")");
  }
  mode = 1;
  outlet(0,"active",1)
...
function op_mode(k,j){
  if (valid && (j || mode !== 0)) {
    ...
    if (k == 2) {
      ...
      else if (thismov.type == "cam") thismov.message("start");
      recording = 1;
    }
    else if (k == 3 || k == 0) {
      ...
      if (recording) {
        ...
        messnamed(jsarguments[1]+"_to.recorder","stop");
        if (thismov.type == "cam") thismov.message("stop")
        recording = 0; }
    }
